 ...
 * Uses Resend API (fetch-based, free 3000/month).
 */
...
function buildEmailHtml(data: { date: string; listingsToday: number; totalListings: number; totalRevenue: number; recentPayments: any[]; lastAgentRun: any }) {
...
<div style="max-width:600px;margin:0 auto;padding:2rem">
  ...
  <table width="100%" cellpadding="0" cellspacing="12" style="margin-bottom:2rem">
    <tr>
      <td width="33%" style="background:#1a1a1a;border:1px solid #2a2a2a;border-radius:8px;padding:1.2rem;text-align:center">
        <div style="font-size:2rem;color:#c9a84c">${data.listingsToday}</div>
        <div style="color:#888;font-size:.75rem">New Listings Today</div>
      </td>
      <td width="33%" style="background:#1a1a1a;border:1px solid #2a2a2a;border-radius:8px;padding:1.2rem;text-align:center">
        <div style="font-size:2rem;color:#c9a84c">${data.totalListings}</div>
      ...
      <td width="33%" style="background:#1a1a1a;border:1px solid #c9a84c;border-radius:8px;padding:1.2rem;text-align:center">
        <div style="font-size:2rem;color:#c9a84c">$${data.totalRevenue.toFixed(0)}</div>
  ...
  <div style="background:#1a1a1a;border:1px solid #2a2a2a;border-radius:8px;padding:1.5rem;margin-bottom:1.5rem">
    ...
    ${data.lastAgentRun ? `<p style="color:#e8e3d9;margin:0;font-size:.9rem">✅ Agent ran — <strong>${data.lastAgentRun.listings_saved} listings</strong> saved</p>` : `<p style="color:#e05c5c;margin:0;font-size:.9rem">⚠️ Agent has not run yet today.</p>`}
  </div>
  ...
  <p style="color:#444;font-size:.72rem;text-align:center;margin-top:2rem">EliteAssetInsight.com — Founded by Mr. Hamdan</p>
</div></body></html>`
}
...
export default async function handler(req: NextRequest) {
  const isAuthorised =
    ...
    req.headers.get('x-agent-secret') === process.env.AGENT_SECRET

  if (!isAuthorised) return NextResponse.json({ error: 'Unauthorised' }, { status: 401 })
  ...
  const [{ count: listingsToday }, { count: totalListings }, { data: paymentsData }, { data: agentRuns }] = await Promise.all([
  ...
  const html = buildEmailHtml({
    ...
    listingsToday: listingsToday || 0, totalListings: totalListings || 0, totalRevenue,
    recentPayments: payments, lastAgentRun: agentRuns?.[0] || null,
  })

  const RESEND_API_KEY = process.env.RESEND_API_KEY || ''
  if (!RESEND_API_KEY) {
    return NextResponse.json({ ok: true, emailSent: false, note: 'Add RESEND_API_KEY to send emails. Free at resend.com', stats: { listingsToday, totalListings, totalRevenue } })
  }
  ...
  try {
    const emailRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from: 'EliteAssetInsight <onboarding@resend.dev>', to: [OWNER_EMAIL], subject: `📊 Daily Report — ${listingsToday} new listings · $${totalRevenue.toFixed(0)} revenue · ${today}`, html }),
    })
    const emailData = await emailRes.json()
    if (!emailRes.ok) throw new Error(emailData.message || 'Resend error')
    return NextResponse.json({ ok: true, emailSent: true, emailId: emailData.id, to: OWNER_EMAIL })
  } catch (e: any) {
